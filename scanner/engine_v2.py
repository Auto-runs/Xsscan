...
class ScanEngineV2:
    ...
    async def _scan_one(self, target: ScanTarget, waf: Optional[str] = None):
        ...
        # Global dedup: include netloc + path supaya param yang sama di URL berbeda
        # tidak di-skip (BUG FIX: sebelumnya hanya netloc, menyebabkan "Testing 0 payloads"
        # ketika param name sama muncul di multiple URLs pada domain yang sama)
        from urllib.parse import urlparse as _up
        _parsed   = _up(target.url)
        _netloc   = _parsed.netloc
        _path     = _parsed.path
        dedup_key = lambda p: f"{_netloc}:{_path}:{target.param_key}:{p}"
