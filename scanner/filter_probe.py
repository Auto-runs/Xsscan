...
class FilterProbe:
    ...
    def _score(self, matrix: CharacterMatrix) -> float:
        ...
        BUG FIX: sebelumnya hanya menghitung 'survivors', mengabaikan 'encoded'
        chars. Encoded chars masih bisa dieksploitasi via WAF bypass techniques
        (double encoding, unicode normalization, dll), sehingga layak dapat
        partial credit (0.4x dari full weight) agar score tidak under-estimate
        dan exploitable threshold (0.3) tidak ter-miss.
        """
        weights = {
            "tag_open":       0.20,
            "tag_close":      0.10,
        ...
        for label, w in weights.items():
            if matrix.can_use(label):
                score += w              # full credit: survived raw
            elif label in matrix.encoded:
                score += w * 0.4        # partial credit: encoded but bypassable
